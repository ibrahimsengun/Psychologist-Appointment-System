...
export const SectionOne: React.FC<SectionOneProps> = React.memo(
  ({ editor, activeLevels = [1, 2, 3, 4, 5, 6], size, variant }) => {
    ...
    const renderMenuItem = React.useCallback(
      ({ label, element, level, className, shortcuts }: TextStyle) => (
        <DropdownMenuItem
          key={label}
          ...
          aria-label={label}
        >
          {React.createElement(element as keyof JSX.IntrinsicElements, { className }, label)}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <ToolbarButton
            ...
            tooltip="Metin stilleri"
            aria-label="Metin stilleri"
